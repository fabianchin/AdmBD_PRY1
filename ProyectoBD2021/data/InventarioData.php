...
include './domain/Inventario.php';

class InventarioData{


    public function insertStock(Inventario $stock){
        // Crea la conexion
        $con = new ConectionDB(); 
        $conn = $con->conection2(); 
        if( $conn === false) {
            echo 'Fallo la conexion a base de datos en el query de createInventario...';
        ...
        $myparams['idStock'] = $Stockid;
        $myparams['stock'] = $stock;
        ...
        $sql = "EXEC sp_insertar_inventario @idStock = ?, @stock = ?";
